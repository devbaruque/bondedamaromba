-- ########## SCRIPT COMPLETO PARA CONFIGURAÇÃO DO BANCO DE DADOS BONDE DA MAROMBA ##########
-- Execute este script inteiro de uma vez no Editor SQL do Supabase

-- Limpar possíveis tabelas existentes para evitar conflitos
DROP TRIGGER IF EXISTS on_auth_user_created ON auth.users;
DROP FUNCTION IF EXISTS public.handle_new_user() CASCADE;
DROP FUNCTION IF EXISTS public.create_user_profile() CASCADE;
DROP FUNCTION IF EXISTS public.trigger_set_updated_at() CASCADE;

DROP POLICY IF EXISTS "Usuários podem ver apenas seu próprio perfil" ON public.users;
DROP POLICY IF EXISTS "Usuários podem atualizar apenas seu próprio perfil" ON public.users;
DROP POLICY IF EXISTS "Permitir inserção na tabela users" ON public.users;
DROP POLICY IF EXISTS "Permitir inserção anônima na tabela users" ON public.users;
DROP POLICY IF EXISTS "Usuários podem criar seu próprio perfil" ON public.users;
DROP POLICY IF EXISTS "Função de sistema pode criar perfis" ON public.users;

DROP POLICY IF EXISTS "Usuários podem ver apenas seus próprios treinos" ON public.workout_plans;
DROP POLICY IF EXISTS "Usuários podem criar seus próprios treinos" ON public.workout_plans;
DROP POLICY IF EXISTS "Usuários podem atualizar seus próprios treinos" ON public.workout_plans;
DROP POLICY IF EXISTS "Usuários podem deletar seus próprios treinos" ON public.workout_plans;

DROP POLICY IF EXISTS "Usuários podem ver seus exercícios" ON public.exercises;
DROP POLICY IF EXISTS "Usuários podem criar exercícios em seus treinos" ON public.exercises;
DROP POLICY IF EXISTS "Usuários podem atualizar exercícios em seus treinos" ON public.exercises;
DROP POLICY IF EXISTS "Usuários podem deletar exercícios em seus treinos" ON public.exercises;

DROP POLICY IF EXISTS "Usuários podem ver imagens de seus exercícios" ON public.exercise_images;
DROP POLICY IF EXISTS "Usuários podem adicionar imagens em seus exercícios" ON public.exercise_images;
DROP POLICY IF EXISTS "Usuários podem atualizar imagens em seus exercícios" ON public.exercise_images;
DROP POLICY IF EXISTS "Usuários podem deletar imagens em seus exercícios" ON public.exercise_images;

DROP POLICY IF EXISTS "Usuários podem ver apenas seu próprio histórico" ON public.workout_history;
DROP POLICY IF EXISTS "Usuários podem registrar apenas seu próprio histórico" ON public.workout_history;
DROP POLICY IF EXISTS "Usuários podem atualizar apenas seu próprio histórico" ON public.workout_history;
DROP POLICY IF EXISTS "Usuários podem deletar apenas seu próprio histórico" ON public.workout_history;

DROP POLICY IF EXISTS "Usuários podem ver apenas seus próprios logs" ON public.exercise_logs;
DROP POLICY IF EXISTS "Usuários podem registrar apenas seus próprios logs" ON public.exercise_logs;
DROP POLICY IF EXISTS "Usuários podem atualizar apenas seus próprios logs" ON public.exercise_logs;
DROP POLICY IF EXISTS "Usuários podem deletar apenas seus próprios logs" ON public.exercise_logs;

DROP TABLE IF EXISTS public.exercise_logs;
DROP TABLE IF EXISTS public.workout_history;
DROP TABLE IF EXISTS public.exercise_images;
DROP TABLE IF EXISTS public.exercises;
DROP TABLE IF EXISTS public.workout_plans;
DROP TABLE IF EXISTS public.users;

-- Criar storage para armazenamento de imagens
-- Executar apenas se os buckets não existirem (o editor SQL pode não permitir criar buckets)
DO $$
BEGIN
    -- Criar bucket para imagens de workout
    PERFORM storage.create_bucket('workout_images', 
                                 '{"public": true, "cache_control": "3600"}');
    EXCEPTION WHEN OTHERS THEN
        RAISE NOTICE 'Bucket workout_images já existe ou não pode ser criado via SQL';
        
    -- Criar bucket para imagens de exercícios
    PERFORM storage.create_bucket('exercise_images', 
                                 '{"public": true, "cache_control": "3600"}');
    EXCEPTION WHEN OTHERS THEN
        RAISE NOTICE 'Bucket exercise_images já existe ou não pode ser criado via SQL';
END $$;
...
-- Permitir que qualquer usuário chame esta função
...
CREATE POLICY "Permitir inserção na tabela users"
  ON public.users
  FOR INSERT
  TO authenticated
  WITH CHECK (true);

CREATE POLICY "Permitir inserção anônima na tabela users"
  ON public.users
  FOR INSERT
  TO anon
  WITH CHECK (true);
